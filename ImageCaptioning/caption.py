...
def caption_image_beam_search(encoder, decoder, img, word_map, beam_size=3):
    ...
    # img.transpose - 행렬의 행과 열, 축 바꾸는 기능/전치행렬
    img = img / 255.
    # .을 왜 나누는 지는 모르겠음
    # 255 나누는 이유도 모르겠음
    img = torch.FloatTensor(img).to(device)
    # torch.FloatTensor - Tensor의 자료형 선언
    # .to로 tensor의 dtype이나 device를 바꿈
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
                # 평균 및 표준 편차로 텐서 이미지 정규화
    transform = transforms.Compose([normalize])
                # torchvision.transforms.Compose( 변환 )
                # 여러 변환을 함께 구성함(무슨 말인지 모르겠음)
    image = transform(img)  # (3, 256, 256)
    # img를 normalize 하는 것 같음
